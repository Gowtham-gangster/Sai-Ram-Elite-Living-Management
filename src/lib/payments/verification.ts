 ...
 * Executes transactional, idempotent verification and status updates.
 */
export async function processVerifiedPayment(
  ...
  // 1. Locate matching MonthlyPayment
  ...
  // 2. Idempotency Check: check if this transaction was already verified
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.paymentRecord.create({
      data: {
        ...
        notes: `Automated verified UPI payment for ${payment.billingMonth} (Ref: ${referenceId}) - ${isFullSettlement ? 'Full Settlement' : `Partial (Balance: ₹${remainingBalance})`}`,
      },
    });

    // C. Cancel reminders ONLY upon full settlement
    if (isFullSettlement) {
    ...
    // D. In-app Notification for Management
    await tx.notification.create({
      data: {
        type: 'SUCCESS',
        title: isFullSettlement ? 'Rent Payment Verified' : 'Partial Rent Payment Verified',
        message: `Verified rent payment of ₹${amountPaid.toLocaleString('en-IN')} received from ${payment.resident.fullName} (Room ${payment.room.roomNumber}) for ${payment.billingMonth}.${isFullSettlement ? '' : ` Remaining Balance: ₹${remainingBalance.toLocaleString('en-IN')}.`}`,
        linkUrl: `/payments`,
      },
    });
  });

  // E. Trigger Automatic PDF Receipt Generation & Drive Upload
  let receiptResult;
  try {
    const { generateAndUploadReceiptForPayment } = await import('@/lib/receipts/receiptEngine');
    receiptResult = await generateAndUploadReceiptForPayment(payment.id, amountPaid, remainingBalance);
  } catch (receiptErr) {
    console.error('Automatic receipt trigger error (Payment remains processed):', receiptErr);
  }
